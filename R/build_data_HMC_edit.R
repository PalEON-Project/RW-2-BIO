...
build_data <- function(site, dvers, mvers, prefix, 
  ...
  if (census_site){
    ...
    if (site == 'HMC'){
      ...
      # remove tree that only has one census dbh, but it was dead
      census = census[which(census$tree_number != 322),]
    ...
    # if (site == 'HMC'){
      ...
      length(unique(census$tree_number))
      
      length(unique(treeMeta$tree_number))
      
      length(unique(c(unique(census$tree_number), unique(treeMeta$tree_number))))
      
      # create a list of all trees that are present in available data
      allTrees = full_join(census, treeMeta, by = c('tree_number')) %>% 
        mutate(taxon = ifelse(is.na(species.x), species.y, species.x),
               distance = ifelse(is.na(distance.y), distance.x, distance.y), 
               plot = substr(plot.x, 4, 4)) %>% 
        select(tree_number, stat_id, taxon, distance, plot)
      allTrees = allTrees[!(duplicated(allTrees)),]
      allTrees = allTrees %>%
        arrange(stat_id)
      
      allTrees$stat_id[which(is.na(allTrees$stat_id))] = seq(nrow(treeMeta)+1, nrow(treeMeta)+length(which(is.na(allTrees$stat_id))))
      
      census_long$stat_id = as.numeric(plyr::mapvalues(census_long$ID, from = allTrees$tree_number, to = allTrees$stat_id, warn_missing = FALSE))
 ...
 # }
  ...
  incr_data$taxon = sapply(c(1:length(incr_data$id)), 
                           function(ind){
                             treeMeta$species[which(treeMeta$stat_id == incr_data$stat_id[ind])]
                           })
  
  # Find year range for data
  
  year_end = max(c(as.numeric(incr_data$year), census_years), na.rm=TRUE)
  year_start = min(c(as.numeric(incr_data$year), census_years), na.rm=TRUE)
  years = seq(year_start, year_end)
  years = years - cutoff + 1
  
  census_years_stat = census_years - cutoff + 1
  ...
  tree_number_snag = treeMeta[which(treeMeta$status == 'Sn'), 'tree_number']
  foo = census[which(census$tree_number %in% tree_number_snag), ]
  ...
  pdbh$dbh = as.numeric(plyr::mapvalues(pdbh$id, from = treeMeta$ID, to = treeMeta$dbh,
                                        warn_missing = FALSE))
  pdbh$distance = as.numeric(plyr::mapvalues(pdbh$stat_id, from = treeMeta$stat_id, to = treeMeta$distance,
                                             warn_missing = FALSE))
  pdbh$taxon = plyr::mapvalues(pdbh$stat_id, from = treeMeta$stat_id, to = treeMeta$species, 
                               warn_missing = FALSE)
  pdbh$tree_number = plyr::mapvalues(pdbh$id, from = treeMeta$ID, to = treeMeta$tree_number, 
                               warn_missing = FALSE)
  pdbh$status = plyr::mapvalues(pdbh$tree_number, from = treeMeta$tree_number, to = treeMeta$status, 
                                     warn_missing = FALSE)
  
  pdbh[which(pdbh$status == 'Li'), 'year'] = sampling_year - cutoff + 1
  ...
  if (census_site){
    ...
    census_long$year = as.numeric(census_long$year) - cutoff + 1
    ...
    for (tree in 1:length(allTrees$stat_id)){
      ...
      if (in.RW & in.C){
        ...
        lastCensus = max(which(which(years %in% census_years_stat) %in% 
        ...
        if ((which(years %in% census_years_stat)[lastCensus]) < lastData){
          lastyr = lastData
        ...
        # which in this case will just be the last year of data 
          ...
          # if not in last census, use year before first census where the tree is missing and mark for smoothing 
            ...
            lastyr = which(years == (census_years_stat[lastCensus+1] - 1))
      ...
      if (!in.RW & in.C){
        ...
        lastCensus = max(which(which(years %in% census_years_stat) %in% unique(census_long$year[which(census_long$stat_id == stat)])))
