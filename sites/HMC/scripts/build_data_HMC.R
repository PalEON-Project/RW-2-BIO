...
# source("config_HMC")
site = 'HMC'
dataDir = paste0('sites/', site, '/data/raw')
dvers = "v0.1"
...
## read data in
censusFile = 'HMC_census_v2.csv'
treeMetaFile = 'HMC.field.data_UPDATED.csv'

censusFull <- read.csv(paste0(dataDir, '/', censusFile), stringsAsFactors = FALSE)
treeMeta <- read.csv(paste0(dataDir, '/', treeMetaFile), skip = 0, stringsAsFactors = FALSE)
...
rwFiles <- list.files(paste0(dataDir, '/', "rwl"))
rwFiles <- rwFiles[grep(".rwl$", rwFiles)]
rwData <- list()
for(fn in rwFiles) {
    id <- gsub(".rw", "", fn)
    rwData[[id]] <- t(read.tucson(file.path(dataDir, "rwl", fn)))  # rows are tree, cols are times
}
...
idx_no = which(((censusFull$Plot %in% treeMeta$Plot) & (censusFull$No. %in% treeMeta$No.)))
idx_in = which((censusFull$Plot %in% treeMeta$Plot) & (censusFull$Dist. <= 16))

# tree number in paleon plot but not within 16 m radius of plot center 
censusFull[idx_no[which(!(idx_no %in% idx_in))],1:14]

# within 16 m radius of plot center, but not cored
censusFull[idx_in[which(!(idx_in %in% idx_no))],1:14]


idx_in %in% idx_no

## get census trees only in plots
plots = unique(treeMeta$Plot)

census = censusFull[which(censusFull$Plot %in% plots),]
census_keep = as.data.frame(matrix(NA, nrow=0, ncol=ncol(census)))
colnames(census_keep) = colnames(census)
for (i in 1:length(plots)){
  this_plot = plots[i]
  
  treeMeta_sub = treeMeta[which(treeMeta$Plot == this_plot),]
  
  census_keep = rbind(census_keep,
                      census[which(((census$Plot == this_plot) & (census$No. %in% treeMeta_sub$No.)) | ((census$Plot == this_plot) & (census$Dist. <= 16))),])
}
...
foo = merge(census_keep[,1:14], treeMeta, by=c('Plot', 'No.'), all=TRUE)
foo = foo[which(!is.na(foo$D14)),]
foo$diff = foo$D14 - foo$DBH

foo[which(is.na(foo$diff)&(foo$D14>10)),1:14]
...
# stemindex 370 was probably out of radius at 16.0 from center; remove
census = census[which(census$stemindex != 370),]
# stemindex 450 was probably out of radius at 15.6 from center; remove
census = census[which(census$stemindex != 450),]
# stemindex 398 was probably out of radius at 16.0 from center; remove
census = census[which(census$stemindex != 398),]

foo = merge(census[,1:14], treeMeta, by=c('Plot', 'No.'), all=TRUE)
foo = foo[which(!is.na(foo$D14)),]
foo$diff = foo$D14 - foo$DBH
...
# this tree is long dead and has no DBH measurements
census = census[which(census$stemindex != 463),]
...
taxaMatch$species <- toupper(taxaMatch$species)
...
for (i in 1:nrow(incr_data)){
  ...
  if (any(which((census$No. == treeMetaNo[i]) & (census$Plot == treeMetaPlot[i])))){
    ...
    if (length(which((census$No. == treeMetaNo[i]) & (census$Plot == treeMetaPlot[i])))>1){
      print(i)
      print(census[which((census$No. == treeMetaNo[i]) & (census$Plot == treeMetaPlot[i])),])
    }
...
pdbh$distance = treeMeta$Distance[match(pdbh$TreeID, treeMeta$TreeID)]
...
# get the site number
site_data    <- data.frame(stat_id=dbh$stat_id, site=dbh$plot_paleon,  census_id=dbh$stemindex, plot=dbh$plot)#census_id=dbh$tree_id, stemindex=dbh$stemindex, plot=dbh$plot)
...
taxon = taxaMatch[match(toupper(census[match(site_data$stat_id, census$stat_id), 'Sp']), taxaMatch$species), 'taxon']
...
save(N_inc, N_dbh, N_pdbh, N_X, N_D, N_trees, N_years, N_sites,
     ...
     # N_saplings, sap2x, not_sap2x, max_size, sapling_tree_id, sapling_year_id,
     first_ti, cs_last_ti, dbh, pdbh, distance,
...
distance_p = distance[match(trees_p, trees)]
...
dump(c('X_p', 'logX_p', 'D0', 'beta', 'beta_t', 'beta0', 'sig_x_obs', 'sig_d_obs', 'sig_d', 'sig_d_sap',
       'sig_x', 'beta_sd', 'beta_t_sd', 'beta_spp', 'beta_spp_sd', 'beta_slope', 
       'tau2', 'b0', 'b1', 'nu', 'rho'), 
     file=paste0('sites/', site, '/data/tree_data_', site ,'_no_census_NIMBLE_inits_', dvers, '.dump'))
...
dump(c('N_inc', 'N_pdbh', 'N_X_p', 'N_D_p', 'N_trees_p', 'N_years', 'N_sites',
     ...
     file=paste0('sites/', site, '/data/tree_data_', site ,'_no_census_NIMBLE_', dvers, '.dump'))
...
save(N_inc, N_pdbh, N_X_p, N_D_p, N_trees_p, N_years, N_sites,
     ...
     trees, years, m2orient,
     taxon, N_taxa, taxaMatch, open_dbh,
     first_ti_p, cs_last_ti_p, dbh, pdbh, distance_p,
     file=paste0('sites/', site, '/data/tree_data_', site ,'_no_census_NIMBLE_', dvers, '.rdata'))
