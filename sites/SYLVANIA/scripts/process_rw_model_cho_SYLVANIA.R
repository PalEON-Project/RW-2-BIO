...
## This script processes the output of the stat model into a format
## that is generally useful in PalEON and for the PEcAn workflow
## Note: this is not the final data format for SDA with LINKAGES
...
# Set date, site and versions
date = '03July2020' 
site <- 'SYLVANIA'
dvers = '072020'
#mvers = 'v4.0'
suff = 'NOALPHA' # unsure what this means

# sampling correction
## TRUE if you want to account for different size trees being measured
## in different size plots, FALSE if you don't want any correction
samp_fix = TRUE
...
dat = readRDS(paste0(data_dir,'/tree_data_', site, '_STAN_',dvers,'.RDS'))
# dat = readRDS(paste0(data_dir,'/NPP_STAT_MODEL_ROOSTER.RDS'))
fnames = c(paste0('ring_model_t_pdbh_', site, '_',dvers,'.RDS'))
models = c('Model RW')
...
## These are the sizes of the plots. If your plot was not done with standard
## PalEON protocol, you will likely need to change these
inner_rad = 13
mid_rad   = 20
outer_rad = 30
