...
fnames = c('ring_model_t_pdbh_ROOSTER_v0.1.RDS')
models = c('Model RW')
...
for (i in 1:length(fnames)) {
  ...
  post[[i]]   =   readRDS(file   = paste0(output_dir,'/', fname_model))
}  
...
ggsave(paste0(figure_dir, '/sum_dbh_by_plot_', site,'.pdf'))
...
not_rm = variables %in% c('ab_p_melt','abi_p_melt','output_dir','site',
           ...
           'suff', 'site', 'figure_dir', 'output_dir')
rm = variables[!not_rm]
rm(list=rm)
...
# save file for WIKI
write.csv(preds, file=paste0(output_dir, '/NPP_STAT_MODEL_', site, '_', suff,'.csv'), row.names=FALSE)
saveRDS(preds, paste0(output_dir,'/NPP_STAT_MODEL_',site, '_', suff,'.RDS'))
...
# save file for WIKI
write.csv(dbh_preds, file=paste0(output_dir, '/DBH_STAT_MODEL_', site, '_', suff, '.csv'), row.names=FALSE)
saveRDS(dbh_preds, paste0(output_dir,'/DBH_STAT_MODEL_',site, '_', suff,'.RDS'))

write.csv(msd_p, file=paste0(output_dir,'/MSD_STAT_MODEL_', site, '_', suff, '.csv'), row.names=FALSE)
saveRDS(msd_p, paste0(output_dir,'/MSD_STAT_MODEL_', site, '_', suff, '.RDS'))
#############################################################################################################
...
ggplot() +  
  ...
  ylab("Biomass (Mg/ha)") + 
  xlab('Year')
ggsave(file=paste0(figure_dir, '/AGB_by_plot_',site, '_', suff,'.png'))
...
ggsave(file=paste0(figure_dir,'/AGB_by_plot_',site, '_', suff,'_freey.png'))
...
ggsave(file=paste0(figure_dir,'/AGBI_by_plot_',site, '_', suff,'.png'))
