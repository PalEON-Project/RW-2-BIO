...
# Save
save(tree_dbh, file = 'data/ind_dbh_ba.RData')
...
goose_lastyear <- tree_dbh |>
  dplyr::filter(site == 'GOOSE')
goose_lastyear <- goose_lastyear |>
  dplyr::filter(year == max(goose_lastyear$year)) |>
...
nrp_lastyear <- tree_dbh |>
  dplyr::filter(site == 'NRP')
nrp_lastyear <- nrp_lastyear |>
  dplyr::filter(year == max(nrp_lastyear$year)) |>
  dplyr::mutate(size = dplyr::if_else(dbh_cm < 20, 'small', NA),
                size = dplyr::if_else(dbh_cm >= 20 & dbh_cm < 30, 'med', size),
                size = dplyr::if_else(dbh_cm >= 30, 'large', size)) |>
  dplyr::ungroup() |>
...
rooster_lastyear <- tree_dbh |>
  dplyr::filter(site == 'ROOSTER')
rooster_lastyear <- rooster_lastyear |>
  dplyr::filter(year == max(rooster_lastyear$year)) |>
  dplyr::mutate(size = dplyr::if_else(dbh_cm < 20, 'small', NA),
                size = dplyr::if_else(dbh_cm >= 20 & dbh_cm < 30, 'med', size),
                size = dplyr::if_else(dbh_cm >= 30, 'large', size)) |>
  dplyr::ungroup() |>
...
sylvania_lastyear <- tree_dbh |>
  dplyr::filter(site == 'SYLVANIA')
sylvania_lastyear <- sylvania_lastyear |>
  dplyr::filter(year == max(sylvania_lastyear$year)) |>
  dplyr::mutate(size = dplyr::if_else(dbh_cm < 20, 'small', NA),
                size = dplyr::if_else(dbh_cm >= 20 & dbh_cm < 30, 'med', size),
                size = dplyr::if_else(dbh_cm >= 30, 'large', size)) |>
  dplyr::ungroup() |>
...
harvard_rw_lastyear <- tree_dbh |>
  dplyr::filter(site == 'HARVARD Model RW')
harvard_rw_lastyear <- harvard_rw_lastyear |>
  dplyr::filter(year == max(harvard_rw_lastyear$year)) |>
  dplyr::mutate(size = dplyr::if_else(dbh_cm < 20, 'small', NA),
                size = dplyr::if_else(dbh_cm >= 20, 'large', size)) |>
  dplyr::ungroup() |>
  dplyr::select(tree, plot, taxon, size)

harvard_rw_taxon_ba <- tree_dbh |>
  dplyr::filter(site == 'HARVARD Model RW') |>
