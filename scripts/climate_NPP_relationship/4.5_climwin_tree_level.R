## code for moving window analysis for individual trees at each site
...
save_comb$Date <- paste0("31/10/", save_comb$year)
...
runclimwin <- function(site.nm){
      ...
      ROwin <- slidingwin(xvar = list(ppt = prism_site$PPT2, 
                          ...
                          cinterval = "month", 
                          #cmissing = "method1",
                          range = c(18,0), # go back to previous may at least
                          type = "absolute", 
                          refday = c(31, 10), # october 31st is reference date now
                          stat = "mean", 
                          func = "lin") 
...
#####################################################################################################
# do the same but with taxon-level random effects for each tree
# getting an error when I try to put a random effect for the species id
# runclimwin_taxon_RE <- function(site.nm){
#   prism_site <- prism_long %>% filter(loc %in% site.nm)
#   comb_site <- save_comb %>% filter(site %in% site.nm)
#   comb_site $taxon_id <- as.numeric(as.factor(comb_site $taxon)) # get numeric ids for each taxon
# 
#   ROwin <- slidingwin(xvar = list(ppt = prism_site$PPT2,
#                                   tave = prism_site$Tmean2,
#                                   tmin = prism_site$Tmin2,
...
#                       cdate = prism_site$Date,
#                       bdate = comb_site$Date,
#                       baseline = lm(residual_AGBI ~ 1 + (1 | taxon_id)  , data = comb_site),
#                       cinterval = "month",
#                       #cmissing = "method1",
#                       range = c(18,0), # go back to previous may at least
#                       type = "absolute",
...
#                      Tmin.heat, Tmax.heat,
#                      VPDmax.heat, align = "hv", ncol = 3)
#   ggsave(height = 8.5, width = 12, units = "in", paste0("out/climwin/AGBI_tree/AGBI_tree_climwin_",site.nm,"taxon_RE.png"))
# 
#   # save all the outputs for model selection in systematic way:
#   save(PPTOutput,
#        TaveOutput,
...
#        TmaxOutput,
#        VPDmaxOutput,
#        file = paste0("out/climwin/AGBI_tree/deltaAICc_AGBI_clim_",site.nm, "taxon_RE.Rdata"))
# }
# 
# lapply(unique(save_comb$site),runclimwin_taxon_RE)
# 
# ########################################################################################
# # read in the output and summarise the top models for each site
# ########################################################################################
